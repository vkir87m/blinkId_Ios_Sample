...
class ViewController: UIViewController {
    ...
    var mrtdRecognizer : MBIndonesiaIdFrontRecognizer?
    ...
    override func viewDidLoad() {
        ...
        MBMicroblinkSDK.sharedInstance().setLicenseResource("license", withExtension: ".mblic", inSubdirectory: "", for: Bundle.main)
    }

    @IBAction func didTapScan(_ sender: AnyObject) {
        
        // To specify we want to perform MRTD (machine readable travel document) recognition, initialize the MRTD recognizer settings
        self.mrtdRecognizer = MBIndonesiaIdFrontRecognizer()
        self.mrtdRecognizer?.returnSignatureImage = true;
...
extension ViewController: MBDocumentOverlayViewControllerDelegate {
    ...
    func documentOverlayViewControllerDidFinishScanning(_ documentOverlayViewController: MBDocumentOverlayViewController, state: MBRecognizerResultState) {
        ...
        if (self.mrtdRecognizer?.result.resultState == MBRecognizerResultState.valid) {
            ...
            let fullDocumentImage = self.mrtdRecognizer?.result.fullDocumentImage?.image
            print("Got MRTD image with width: \(String(describing: fullDocumentImage?.size.width)), height: \(fullDocumentImage?.size.height)")
